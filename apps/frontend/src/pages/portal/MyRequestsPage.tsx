...
export function MyRequestsPage() {
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        <p className="mb-2 text-sm font-medium text-muted-foreground">{t('requests.chooseType')}</p>
        <div className="grid grid-cols-2 gap-3">
          {absenceTypes.map((absenceType) => (
            <button
              key={absenceType.id}
              type="button"
              onClick={() => openForm(absenceType.id)}
              className="flex flex-col items-start gap-2 rounded-2xl border border-border bg-card p-4 text-left shadow-xs active:scale-[0.97] transition-transform"
            >
              <span
                className="flex size-9 items-center justify-center rounded-full"
                style={{ backgroundColor: `${absenceType.color}26` }}
              >
      ...
      <div>
        <p className="mb-2 text-sm font-medium text-muted-foreground">{t('requests.history')}</p>
        ...
        {!requestsQuery.isLoading && !requestsQuery.isError && requests.length === 0 && (
          <Card className="rounded-2xl">
            <CardContent className="py-10 text-center text-sm text-muted-foreground">{t('requests.noneYet')}</CardContent>
          </Card>
        )}
                    ...
                    <div className="min-w-0">
                      ...
                      {request.status !== 'PENDING' && request.reviewComment && (
                        <p className="mt-1 text-xs text-muted-foreground">
                          {t('requests.managerNotePrefix', { note: request.reviewComment })}
                        </p>
                      )}
                    </div>
      ...
      <RequestFormDialog open={formOpen} onOpenChange={setFormOpen} initialAbsenceTypeId={initialAbsenceTypeId} />
